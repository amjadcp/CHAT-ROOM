...
// Autoplay Management
let audioContext = null; // Web Audio API context for unblocking autoplay
 ...
 */
async function init() {
  // Request microphone permission first (shows browser native prompt)
...
async function requestInitialPermissions() {
  ...
  try {
    ...
    // Request microphone - shows browser's native prompt
    ...
    console.log("✅ Audio system initialized - remote audio will autoplay");
  } catch (error) {
    console.error("❌ Permission denied or error:", error);

    // Show user-friendly message
    showSimpleNotification(
      "⚠️ Audio setup error. You may need to manually enable audio when joining a call.",
      "warning",
      5000
    );
  }
...
// ============================================================================
// ERROR MESSAGES (Simple inline notifications)
// ============================================================================
...
function handleRemoteTrack(event, userId) {
  ...
  if (!audioEl) {
    ...
    audioEl.controls = false; // Remove controls in production
    audioEl.volume = 1.0;
  ...
  if (event.streams && event.streams[0]) {
    ...
    if (playPromise !== undefined) {
      playPromise
        .then(() => {
          console.log("✅ Audio playing automatically for:", userId);
        })
        .catch((err) => {
          console.error("❌ Autoplay blocked for:", userId, err);
...
function showAudioBlockedPrompt(userId) {
  ...
  prompt.innerHTML = `
    <h3>🔊 Enable Audio</h3>
    <p>Click to hear ${userId.substring(0, 8)}...</p>
    <button id="enableAudioNow" style="
      padding: 10px 20px;
      background: #4CAF50;
      color: white;
      border: none;
  ...
  document.getElementById("enableAudioNow").onclick = () => {
...
function updateRoomDisplay() {
  ...
  if (myRoom.size === 0) {
    ...
    myRoom.forEach((userId) => {
      ...
      const userName = userLi
        ? userLi.querySelector("span").textContent
        : "User";
...
async function processPendingCandidates(userId) {
  ...
  if (candidates.length > 0) {
    console.log(
      `Processing ${candidates.length} pending candidates for:`,
      userId
    );
...
function handleTouchMove(e) {
  ...
  if (touchDragState.clone) {
    touchDragState.clone.style.left =
      touch.clientX - touchDragState.clone.offsetWidth / 2 + "px";
